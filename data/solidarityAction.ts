...
export const formatSolidarityAction = async (
  record: SolidarityActionAirtableRecord
  ...
  for (const countryCode of action.fields.countryCode || []) {
    ...
    if (action.fields.LocationData) {
      ...
      const _data = await geocodeOpenStreetMap(
        action.fields.Location!,
        countryCode
      );
      // @ts-ignore
      const { data, error } =
        openStreetMapReverseGeocodeResponseSchema.safeParse(_data);
      if (error) {
...
export function actionToFeature(
  action: SolidarityAction
): GeoJSON.Feature<GeoJSON.Point, SolidarityAction> {
  //
  return {
    type: "Feature",
    geometry: {
...
const fields: Array<keyof SolidarityActionAirtableRecord["fields"]> = [
  "hasPassedValidation",
  "slug",
  "companyName",
  "organisingGroupName",
...
//
export async function getAllSolidarityActions({
  filterByFormula = "",
  ...selectArgs
}: QueryParams<SolidarityActionAirtableRecord["fields"]> = {}): Promise<
  Array<SolidarityActionAirtableRecord>
> {
...
export async function getLiveSolidarityActions({
  filterByFormula,
  ...selectArgs
}: QueryParams<SolidarityActionAirtableRecord["fields"]> = {}): Promise<
  Array<SolidarityAction>
> {
  const airtableRecords = await getAllSolidarityActions({
    ...
    view: env
      .get("AIRTABLE_TABLE_VIEW_SOLIDARITY_ACTIONS")
      .default("Live")
      .asString(),
    ...selectArgs,
  });
