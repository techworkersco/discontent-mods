...
export function Map({
  ...
  const countryCounts = useMemo(() => {
    ...
    // Todo: i think we can generate this color scale in a much simpler automatic
    const colorScale = scalePow()
      .exponent(0.5)
      .domain([min(domain), median(domain), max(domain)] as number[])
      .range([
        theme`colors.gwRedLighter`,
        theme`colors.gwRedLight`,
        theme`colors.gwRed`,
        theme`colors.gwRedDark`,
      ] as any);
      ...
      >
        <ReactMapGL
          ...
          accessToken={env
            ...
            .default(
              "pk.eyJ1IjoicmVyb290aW5nMjA0MCIsImEiOiJjbDl0eHN1cTkwYW55M29uemxmMTR0OTVjIn0.IeLswEOZzPxx6gbxeGSNaw"
            )
            .asString()}
          mapStyle={env
            .get("NEXT_PUBLIC_MAPBOX_STYLE_URL")
            .default("mapbox://styles/rerooting2040/cl9tzuyjo002114mi4hqe1nd6")
            .asString()}
